...
class SiteController extends Controller
{
    ...
    protected function home()
    {
        ...
        $new_product = Product::where('status', '!=', '0')->Orderby('created_at', 'desc')->take(8)->get();
    ...
    public function all_product()
    {
        if (isset($_GET['ten'])) {
            ...
            $list_product = Product::where('status', '2')->Orderby('name', $ten)->paginate($this->paginate);
        } elseif (isset($_GET['gia'])) {
            $gia = $_GET['gia'];
            $gia = $gia == 'giam' ? 'desc' : 'asc';
            $list_product = Product::where('status', '2')->Orderby('price', $gia)->paginate($this->paginate);
        } else {
            $list_product = Product::where('status', '2')->Orderby('created_at', 'desc')->paginate($this->paginate);
        }
    ...
    protected function product_category($slug)
    {
        ...
        if (isset($_GET['ten'])) {
            $ten = $_GET['ten'];
            $ten = $ten == 'z-a' ? 'desc' : 'asc';
            $list_product = Product::where('status', '2')->whereIn('category_id', [$cat->id])->Orderby('name', $ten)->paginate($this->paginate);
        } elseif (isset($_GET['gia'])) {
            $gia = $_GET['gia'];
            $gia = $gia == 'giam' ? 'desc' : 'asc';
            $list_product = Product::where('status', '2')->whereIn('category_id', [$cat->id])->Orderby('price', $gia)->paginate($this->paginate);
        } else {
            $list_product = Product::where('status', '=', '1')->whereIn('category_id', [$cat->id])->OrderBy('created_at', 'desc')->paginate($this->paginate);
        }
        return view('frontend.product_category', compact('list_product', 'cat'));
    }
    protected function product_brand($slug)
    {
        $brand = Brand::where([['status', '=', '2'], ['slug', '=', $slug]])->first();
        $list_category = Category::where('status', '2')->orderBy('created_at', 'desc')->get();
        $list_brand = Brand::where('status', '2')->orderBy('created_at', 'desc')->get();
        if (isset($_GET['ten'])) {
            $ten = $_GET['ten'];
            $ten = $ten == 'z-a' ? 'desc' : 'asc';
            $list_product = Product::where('status', '2')->whereIn('brand_id', [$brand->id])->Orderby('name', $ten)->paginate($this->paginate);
        } elseif (isset($_GET['gia'])) {
            $gia = $_GET['gia'];
            $gia = $gia == 'giam' ? 'desc' : 'asc';
            $list_product = Product::where('status', '2')->whereIn('brand_id', [$brand->id])->Orderby('price', $gia)->paginate($this->paginate);
        } else {
            $list_product = Product::where('status', '=', '1')->whereIn('brand_id', [$brand->id])->OrderBy('created_at', 'desc')->paginate($this->paginate);
        }
    ...
    protected function post_detail($post_single)
    {
        ...
        $list_post = Post::where([['status', '2'], ['topic_id', $topic->id], ['id', '!=', $post_single->id]])->Orderby('created_at', 'desc')->take(4)->get();
        return view('frontend.post_detail', compact('list_post', 'post_single'));
    }
    protected function post_page($slug)
    {
        $page = Post::where([['slug', '=', $slug], ['status', '=', 2]])->first();
